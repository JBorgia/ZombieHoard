...
public class LoginController {
	...
	public ModelAndView login(@Valid Customer customer, Errors errors) {
		ModelAndView mv = new ModelAndView();
		// customers.JPQLQuery();
		// if(customer.getAccessLevel() == CustomerAccessLevel.LOGOUT)
		...
		if (errors.getErrorCount() != 0 || validEmail == null || validPassword == null || !validEmail
				|| !validPassword) {
			// If there are any errors, return the login form.
			// if(!validEmail) mv.addObject("email", "Invalid email: \"" +
			// customer.getEmail() + "\" entered.");
			if ((validEmail == null || !validEmail) && (validPassword != null && validPassword))
				errors.rejectValue("email", "login.combination");
			else if (validEmail == null || !validEmail)
				errors.rejectValue("email", "login.email");

			if ((validPassword == null || !validPassword) && (validEmail != null && validEmail))
				errors.rejectValue("password", "login.combination");
			else if (validPassword == null || !validPassword)
				errors.rejectValue("password", "login.password");
			// if(!validPassword) mv.addObject("password", "Invalid password
			// entered.");
			mv.setViewName("login.jsp");
			return mv;
		}
		...
		if (cust.getAccessLevel().equals("ADMIN")) {
			...
			mv.setViewName("ViewCart.do");
		}

		System.out.println(customer);
		// zombieDAO.fetchCart().setCustomer(cust);

		zombieDAO.persistCart(zombieDAO.fetchLastActiveCart(cust));
	...
	public ModelAndView logout(@ModelAttribute("customer") Customer customer, HttpServletRequest request,
		...
		if (customer.getAccessLevel().equals(UserAccessLevel.LOGOUT)) {
			...
			RequestDispatcher rd = request.getRequestDispatcher("Refresh.do");
			try {
	...
	public ModelAndView register(@Valid Customer customer, Errors errors) {
		...
		if (customer.getEmail() == null) {
			mv.addObject("register", "register.do");
			mv.setViewName("register.jsp");
			// System.out.println(customer);

		} else if (errors.getFieldErrorCount() != 0 || registerEmail == null || !registerEmail
				|| registerPassword == null || !registerPassword || registerAge == null || !registerAge
				|| registerHeight == null || !registerHeight || registerWeight == null || !registerWeight
				|| registerZipcode == null || !registerZipcode) {
			mv.addObject("register", "register.do");
			mv.setViewName("register.jsp");
			if (registerEmail == null)
				errors.rejectValue("email", "Email.customer.email");
			if (registerPassword == null || !registerPassword)
				errors.rejectValue("password", "Password.customer.password");
			if (registerAge == null)
				errors.rejectValue("age", "Age.customer.age");
			if (registerHeight == null)
				errors.rejectValue("height", "Height.customer.age");
			if (registerWeight == null)
				errors.rejectValue("weight", "Weight.customer.weight");
			if (registerZipcode == null)
				errors.rejectValue("zipcode", "Zipcode.customer.zipcode");
			else if (!registerZipcode)
				errors.rejectValue("zipcode", "Pattern.customer.zipcode");
			...
			// customers.createAccount(new Customer(customer.getEmail(),
			// customer.getFirstName(), customer.getMiddleName(),
			// customer.getLastName(), customer.getPassword(),
			// customer.getAge(), customer.getHeight(), customer.getWeight(),
			// customer.getZipcode(), UserAccessLevel.BASIC));
		} else {
			mv.addObject("register", "register.do");
			mv.addObject("Result", "Unsuccessful Registration.");
	...
	public ModelAndView updateAccountPage(@Valid Customer customer, Errors errors,
			@RequestParam(name = "update", required = false) Integer update) {
		ModelAndView mv = new ModelAndView();

		if (update != null) {
			if (update < 0)
				mv.addObject("update", Math.abs(update));
